...
def test_load_whitout_flatten(
        spark_session_mock, json_string_parser_mock, struct_flattener_mock
):
    reader = MultilineJsonReader(
        spark_session_mock, json_string_parser_mock, struct_flattener_mock
    )
    spark_session_mock.read.text.return_value.collect.return_value = [
        {"value": "mocked file content"}
    ]
